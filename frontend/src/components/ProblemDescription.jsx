...
function ProblemDescription({ problem, currentProblemId, onProblemChange, allProblems }) {
  return (
    <div className="h-full overflow-y-auto bg-[#0d1117]">
      {/* HEADER SECTION */}
      <div className="p-4 sm:p-6 bg-[#161b22] border-b border-[#30363d] sticky top-0 z-10">
        <div className="flex items-start justify-between gap-2 mb-3 flex-wrap">
          <h1 className="text-xl sm:text-2xl font-bold text-white font-nunito">{problem.title}</h1>
          <span className={`badge ${getDifficultyBadgeClass(problem.difficulty)} px-3 py-1 shrink-0`}>
            {problem.difficulty}
          </span>
        </div>
      ...
      <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
        ...
        <div className="bg-[#161b22] rounded-lg border border-[#30363d]">
          <div className="px-4 py-3 border-b border-[#30363d] flex items-center gap-2">
            <BookOpenIcon className="size-4 text-[#6217d2] shrink-0" />
          ...
          <div className="p-4 space-y-3 text-sm leading-relaxed">
            <p className="text-[#c9d1d9] font-nunito break-words">{problem.description.text}</p>
            {problem.description.notes.map((note, idx) => (
              <p key={idx} className="text-[#c9d1d9] font-nunito break-words">
                {note}
              </p>
            ))}
          </div>
        ...
        <div className="bg-[#161b22] rounded-lg border border-[#30363d]">
          <div className="px-4 py-3 border-b border-[#30363d] flex items-center gap-2">
            <ListIcon className="size-4 text-[#6217d2] shrink-0" />
          ...
          <div className="p-4 space-y-4">
            {problem.examples.map((example, idx) => (
              <div key={idx} className="space-y-2">
                ...
                <div className="bg-[#0d1117] border border-[#30363d] rounded-md p-3 font-mono text-xs space-y-2 overflow-x-auto">
                  <div className="flex flex-col xs:flex-row gap-1 xs:gap-2">
                    <span className="text-[#6217d2] font-bold xs:min-w-[60px] shrink-0">Input:</span>
                    <span className="text-[#c9d1d9] break-words">{example.input}</span>
                  </div>
                  <div className="flex flex-col xs:flex-row gap-1 xs:gap-2">
                    <span className="text-[#FFCC00] font-bold xs:min-w-[60px] shrink-0">Output:</span>
                    <span className="text-[#c9d1d9] break-words">{example.output}</span>
                  </div>
                  {example.explanation && (
        ...
        <div className="bg-[#161b22] rounded-lg border border-[#30363d]">
          <div className="px-4 py-3 border-b border-[#30363d] flex items-center gap-2">
            <AlertCircleIcon className="size-4 text-[#6217d2] shrink-0" />
            ...
            <ul className="space-y-2 text-[#c9d1d9] text-sm font-nunito">
              {problem.constraints.map((constraint, idx) => (
                <li key={idx} className="flex gap-2 items-start">
                  <span className="text-[#6217d2] mt-0.5 shrink-0">•</span>
                  <code className="text-xs bg-[#0d1117] px-2 py-0.5 rounded border border-[#30363d] break-all">
                    {constraint}
                  </code>
                </li>
              ))}
