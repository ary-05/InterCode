...
int main() {
    ...
    printVector(twoSum(nums1, 9)); // Expected: [0, 1]
    
    vector<int> nums2 = {3, 2, 4};
    printVector(twoSum(nums2, 6)); // Expected: [1, 2]
    
    vector<int> nums3 = {3, 3};
    printVector(twoSum(nums3, 6)); // Expected: [0, 1]
...
int main() {
    ...
    difficulty: "Hard",
    category: "Array • Sliding Window • Monotonic Queue",
    description: {
      text: "You are given an array of integers nums and an integer k. There is a sliding window of size k which is moving from the very left of the array to the very right. You can only see the k numbers in the window. Each time the sliding window moves right by one position.",
      notes: [
        "Return an array of the maximum value in each sliding window.",
        "A naive solution will be too slow.",
      ],
    },
    examples: [
      {
        ...
        output: "[3,3,5,5,6,7]",
        explanation:
          "The maximums for each window are: [1,3,-1]→3, [3,-1,-3]→3, [-1,-3,5]→5, [-3,5,3]→5, [5,3,6]→6, [3,6,7]→7.",
      },
      {
        input: "nums = [1], k = 1",
        output: "[1]",
...
int main() {
    ...
    printVector(maxSlidingWindow(nums1, 3)); // Expected: [3,3,5,5,6,7]
    
    vector<int> nums2 = {1};
    printVector(maxSlidingWindow(nums2, 1)); // Expected: [1]
